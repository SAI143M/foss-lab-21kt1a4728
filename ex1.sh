#shellsscript to show various system configurations
...
case $op in
    ...
    3) echo "home directory:- $HOME"
        ;;
    4) echo "operating system type & it version: $(cat /proc/version)"
        ;;
    5) echo "current path setting:- $PATH"
        ;;
    6) echo "current working directory:- $PWD"
        ;;
    7) echo "current logged number of users:- $who|wc -1"
        ;;
    *) echo "invalid option"
        ;;
esac 
